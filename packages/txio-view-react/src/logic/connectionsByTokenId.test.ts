...
describe("tokenIdsGroupedByinternalId", () => {
  it("should group tokenId by id", () => {
    ...
    const data = {
      inputs: [
        { internalId: "In-A", assets: [{ tokenId: "T1" }, { tokenId: "T2" }] },
        { internalId: "In-B", assets: [{ tokenId: "T1" }] },
      ],
      outputs: [
        { internalId: "Out-1", assets: [{ tokenId: "T2" }] },
        { internalId: "Out-2", assets: [{ tokenId: "T1" }, { tokenId: "T2" }] },
      ],
    };
    const inputResult = tokenIdsGroupedByinternalId(data.inputs);
    expect(inputResult).toEqual(expectedForInput);

    const outputResult = tokenIdsGroupedByinternalId(data.outputs);
